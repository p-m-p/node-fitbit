...
cp.getActivities = function (options, callback) {
  this._getResource('activities', Activities, options, callback);
};

// Fetches the sleep for a given date set in `options`. If
// `options.date` is not supplied then sleep for the current day will
// be supplied to `callback`
//
// `callback` parameters are `error`, `sleep` and `data` where `sleep`
// is a Sleep resource object and data is the raw response data
cp.getSleep = function (options, callback) {
  this._getResource('sleep', Sleep, options, callback);
};
...
cp._getResource = function (type, Resource, options, callback) {
  ...
  this.apiCall(
    ...
    , function (err, data) {
        ...
        callback.call(_undefined, err, new Resource(data), data);
      }
  );
};
